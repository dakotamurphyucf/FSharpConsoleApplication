namespace FSharpConsoleApplication
...
module CommandLoop =
     ...
     type Actions = Map<string, string -> unit>

     let runCommandLoop tag quitlist actions defaultAction = 
          let action tag quitList = 
              Console.Write (("\n") + tag)
              match Console.ReadLine() with
              | line when List.contains (line.ToLower()) quitList -> None
              | line -> Some line
    
          let run (actions: Actions) itemOp = 
               match itemOp with
               | Some item -> 
                    match Map.tryFind item actions with 
                    | Some action -> action item
                    | None -> defaultAction item
                    None
               | None -> Some "quit"

          let readlines = Seq.initInfinite (fun _ -> action tag quitlist)

          Seq.choose (run actions) readlines |> ignore
